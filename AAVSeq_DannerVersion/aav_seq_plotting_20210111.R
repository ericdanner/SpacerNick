...
# Data import -------------------------------------------------------------
files <- list.files("bed_files/global/", pattern = ".bed", full.names = TRUE)
...
for (i in files) {
  if (file.info(i)$size > 0) {
    ...
    filename <- unlist(str_split(filename, pattern = "_"))[3]
    sample <- unlist(str_split(filename, pattern = "\\."))[1]
    rep <- unlist(str_split(filename, pattern = "\\."))[2]
...
rm(maps.tmp, i, filename, sample, rep, files)
...
maps.suspicious <- maps.suspicious %>%
  mutate(length = end - start + 1)
write.table(maps.suspicious, "ambiguous.mappings.local.031120.txt",
            sep = "\t", row.names = FALSE, quote = FALSE)

contigs <- contigs %>%
  filter(length < 2500, !str_detect(chrom, "_"))
contigs <- contigs %>% unite(samplerep, c(sample, rep))
contigs.agg <- contigs %>% group_by(samplerep) %>%
  summarise(paired.read.count = n())
contigs <- contigs %>% separate(samplerep, c("sample", "rep"))
...
predictions <- read.table("crispr_gold_predictions.txt",
                          header= TRUE, sep = "\t",
                          stringsAsFactors = FALSE)
...
files <- list.files("clusters_local/", pattern = ".txt", full.names = TRUE)
...
for (i in files) {
  if (file.info(i)$size > 0) {
    ...
    rep <- unlist(str_split(filename, pattern = "_"))[2]
...
clusters <- read.table("N707_N505_110121.txt",
                       header = TRUE, sep = "\t",
                       stringsAsFactors = FALSE)
clusters$sample <- "N707_N505"
clusters1 <- read.table("N708_N505_110121.txt",
                       header = TRUE, sep = "\t",
                       stringsAsFactors = FALSE)
clusters1$sample <- "N708_N505"
...
write.table(clusters,
            file = "clustered_reads_offtargets_global_20210111.txt",
            sep = "\t",
